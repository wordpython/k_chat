...
public class clientTwo extends Frame implements ActionListener{
	...
	clientTwo(String username) {
		...
		try {
			...
			skt = new DatagramSocket(9002);
			sendMessageToServer(address,username,null,9002,"yes");//将客户端本地计算机名发送给服务器
		...
		addWindowListener(new WindowAdapter(){
			public void windowClosing(WindowEvent e) {
				...
				sendMessageToServer(address,username,null,9002,"no");
				System.exit(0);
			}
		});
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(message!=null && !message.equals("")) {
			...
			jta2.setText(null);
			sendMessageToServer(address,username,message,9002,"yes");
		}
	}
	private void sendMessageToServer(String hostName,String username,String message,int port,String state) {//发送消息的方法
		try {
			...
			//将字节数组的数据放入数据包
			DatagramPacket pkt = new DatagramPacket(buf, buf.length,InetAddress.getByName("192.168.191.5"),8000);//DESKTOP-PMALJHC
			skt.send(pkt);
			skt.close();
