	...
	private serverOne() {
		try {
			...
			while(true) {
				...
				System.out.println("11111111111111111123");
				//判断是消息还是登录信息
...
//					map.put(user.hostName, user.username);//这里用错了就会导致其他电脑收不到
	...
	private void sendAllNumberTo(User user) {//将在线人数发送过去
		try {
			for (String username : map.values()) {
				System.out.println(mapHostName.get(username)+"");
				...
				skt.send(pkt);
				skt.close();
				System.out.println("发送人数成功："+username+" :"+address);
			}
