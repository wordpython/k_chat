...
public class clientOne extends Frame{
	...
	private static JTextPane jta2=new JTextPane();//编辑信息区
...
//			jll[i].setSize(icon.getIconWidth(), icon.getIconHeight());
		...
		bt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				sendMessageToServer(address,username,str,al,9000,"yes");
				al.clear();
				jta2.setText(null);
			}
		});
		try {
			...
			sendMessageToServer(address,username,"",al,9000,"yes");//将客户端本地计算机名发送给服务器
		...
		addWindowListener(new WindowAdapter(){
			public void windowClosing(WindowEvent e) {
				...
				//用户关闭之前发送关闭信息给服务器
				sendMessageToServer(address,username,"",al,9000,"no");
				System.exit(0);
			}
		});
...
//			User user=new User("1", username,message,0,port,state,al);//发送的对象
			User user=new User("","我我我",message,al,port,state,0);//"1"时：$BlockDataInputStream.peekByte ?为什么username就可以
			//对象流  写  对象输出流
	        //内存流（基类流）
			...
			skt.send(pkt);
			skt.close();
			System.out.println("发送消息: "+hostName+" 信息为:"+message+" 表情数:"+al.size());
