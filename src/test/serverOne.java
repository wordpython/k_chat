	...
	private serverOne() {
		try {
			...
			while(true) {
				...
				//判断是消息还是登录信息
				System.out.println(user.state+" : "+user.username+" 消息："+user.message+" 端口号:"+user.port+" 计算机名:"+user.hostName);
				if(!user.message.equals("")) {
					System.out.println("消息不为空："+user.message+" 表情数:"+user.al.size());
					user.numbers=-1;
					sendToAll(user);
...
//					user.setNumbers(map);//对象里面有对象，就会报错，
					User user1=new User("", "","",0,map.size());//发送的对象
